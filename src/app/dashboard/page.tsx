        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <FollowupBell />
              <a href="/analytics" className="px-3 py-1 bg-indigo-100 text-indigo-800 rounded-full text-sm font-medium hover:bg-indigo-200 transition-colors cursor-pointer">
                Analytics
              </a>
              <a href="/sales" className="px-3 py-1 bg-emerald-100 text-emerald-800 rounded-full text-sm font-medium hover:bg-emerald-200 transition-colors cursor-pointer">
                Sales Admin
              </a>
              <button
                onClick={async () => {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => { const next = shiftDay(viewDate, 1); if (next <= getTodayIST()) setViewDate(next); }}
                ...
                return (
                  <div key={chat.chat_id} className="bg-white rounded-xl shadow-sm border-2 border-green-300 overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mb-2"><span className="text-xs text-gray-500">Name</span><div className="text-black font-medium">{visitorInfo?.name || chat.visitor_name || "Not provided"}</div></div>
                      <div className="mb-2"><span className="text-xs text-gray-500">Phone</span><div className="text-black font-medium">{visitorInfo?.phone || chat.visitor_phone || "Not provided"}</div></div>
                      <div className="mb-2">
                        <span className="text-xs text-gray-500">Location</span>
                        <div className="text-black">{getCountryFlag(chat.visitor_country)} {chat.visitor_city}, {chat.visitor_country}{visitorInfo?.location && ` (${visitorInfo.location})`}</div>
                      </div>
                      <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500">Started: {formatTime(chat.started_at)}</div>
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <button onClick={() => toggleHistory(chat.chat_id)} className="text-xs text-black font-semibold flex items-center gap-1 hover:underline">
                        ...
                        {historyOpenIds.has(chat.chat_id) && (
                          <div className="mt-2 space-y-2 max-h-64 overflow-y-auto">
                            {chat.sales ? renderLeadCard(chat.sales, `Current · ${fmtSavedAt(chat.sales.updated_at || "")}`, 0) : <p className="text-xs text-black/50 text-center py-2">No sales info yet</p>}
                            {(historyCache[chat.chat_id] || []).map((h) => renderLeadCard(h, fmtSavedAt(h.saved_at), h.id))}
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-black">
              {displayDate(viewDate)} Chats{searchQuery || selectedProperty !== "all" ? ` (${filteredTranscripts.length} filtered)` : ` (${total})`}
            </h2>
            {transcripts.length > 0 && (
              <button onClick={downloadExcel} className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-colors text-sm">
                ⬇ Download CSV
              </button>
            )}
          </div>
          ...
          {filteredTranscripts.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-500 text-sm mt-1">
                {searchQuery || selectedProperty !== "all" ? "Try adjusting your filters" : isToday ? "Chats will appear here after they end" : "No chats were recorded on this day"}
              </p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                return (
                  <div key={transcript.chat_id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-3">
                      ...
                      <div className="text-white/80 text-xs mt-1">{formatDate(transcript.created_at)} • {formatShortTime(transcript.created_at)}</div>
                    </div>
                    <div className="p-4">
                      {transcript.sales && (
                        <div className="mb-3 flex flex-wrap gap-1.5">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${SALES_STATUS_COLORS[transcript.sales.status] || SALES_STATUS_COLORS.New}`}>{transcript.sales.status}</span>
                          {transcript.sales.converted && <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-green-100 text-green-700">✓ Converted</span>}
                          {transcript.sales.premium_collected && <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-emerald-100 text-emerald-700">💰 Collected</span>}
                          {transcript.sales.updated_in_crm && <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-blue-100 text-blue-700">CRM ✓</span>}
                          {transcript.sales.premium_amount != null && <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-gray-100 text-gray-700">₹{transcript.sales.premium_amount}</span>}
                        </div>
                      )}
                      <div className="mb-3">
                      ...
                      {transcript.messages && transcript.messages.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          <span className="text-xs text-gray-500 uppercase tracking-wide mb-2 block">Messages ({transcript.messages.length})</span>
                              ...
                              return (
                                <div key={msg.id} className="bg-gray-50 rounded-lg p-2">
                                  <div className="flex items-center gap-2 mb-1">
                                    <span className="text-xs px-2 py-0.5 rounded-full bg-gray-200 text-gray-700">{sender.icon} {sender.label}</span>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <button onClick={() => toggleHistory(transcript.chat_id)} className="text-xs text-black font-semibold flex items-center gap-1 hover:underline">
                        ...
                        {historyOpenIds.has(transcript.chat_id) && (
                          <div className="mt-2 space-y-2 max-h-64 overflow-y-auto">
                            {transcript.sales ? renderLeadCard(transcript.sales, `Current · ${fmtSavedAt(transcript.sales.updated_at || "")}`, 0) : <p className="text-xs text-black/50 text-center py-2">No sales info yet</p>}
                            {(historyCache[transcript.chat_id] || []).map((h) => renderLeadCard(h, fmtSavedAt(h.saved_at), h.id))}
                          </div>
                        )}
                      </div>
                    </div>
